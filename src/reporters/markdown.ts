...
function scoreImpactText(finding: Finding): string {
  if (finding.score_category !== "scored") return "score_impact=0";
  if (!finding.score_applied) return `score_impact=0 deduped_or_capped dedupe_key=${finding.score_dedupe_key}`;
...
function nullableScore(score: number | null): string {
...
export function toMarkdown(result: ScanResult): string {
  ...
  if (result.summary.status !== "evaluated") {
    lines.push(
      "",
      "## Not Evaluated / Unsupported Coverage",
      `This repository was not scored because status is \`${result.summary.status}\`.`,
      "Unsupported or non-runtime files can still be listed in coverage, but they do not produce a public V0 badge score."
    );
  }

  lines.push(
    ...section("Scored Findings", scored, "No scored findings."),
    ...section("Preview Findings", preview, "No preview findings."),
    ...section("Advisory Findings", advisory, "No advisory findings.")
  );
  return `${lines.join("\n")}\n`;
}
