import type { Finding, ScanConfig, ScanCoverage, ScanSummary, Severity } from "./types.js";
...
const rulePenaltyCaps: Record<string, number> = {
  "MCP-SEC-001": 50,
  "MCP-SEC-003a": 20,
  "MCP-SEC-003b": 20,
  "MCP-SEC-005": 20,
...
function impactFor(severity: Severity, confidence: Finding["confidence"], threshold: ScanConfig["confidence_threshold"]): number {
  if (confidence === "low") return 0;
  if (!confidenceMeetsThreshold(confidence, threshold)) return 0;
...
function normalizedRuntimeRoot(filePath: string): string {
...
function routeOrToolIdentifier(finding: Finding): string | undefined {
  ...
  if (bindExpression) return `bind:${bindExpression.replace(/['"`]/g, "")}`;
  return undefined;
}
...
function scoreCategoryFor(finding: Finding, config: ScanConfig): Finding["score_category"] {
  if (!publicV0ScoredRules.has(finding.rule_id)) {
    return finding.confidence === "low" ? "advisory" : "preview";
  }
  if (finding.confidence === "low" || !confidenceMeetsThreshold(finding.confidence, config.confidence_threshold)) return "advisory";
  return "scored";
}

export function applyPresentation(findings: Finding[], config: ScanConfig): Finding[] {
  const prepared = findings.map((finding) => {
    ...
    const configSuppression = config.suppressions.find((suppression) => {
      if (suppression.rule_id !== finding.rule_id) return false;
      if (suppression.fingerprint && suppression.fingerprint !== finding.fingerprint) return false;
      if (suppression.path && suppression.path !== finding.location.path) return false;
      return true;
    });
    const next = { ...finding };
    if (override) {
  ...
  const rawUsedByRule = new Map<string, number>();
  const effectiveUsedByRule = new Map<string, number>();
  const rawDedupe = new Set<string>();
  const effectiveDedupe = new Set<string>();

  return prepared.map((finding) => {
    const next = { ...finding };
    if (next.score_category !== "scored") return next;
...
export function scoreFindings(findings: Finding[], config: ScanConfig, coverage: ScanCoverage): ScanSummary {
  ...
  let rawScore: number | null = evaluated ? Math.max(0, 100 - rawPenalty) : null;
  let effectiveScore: number | null = evaluated ? Math.max(0, 100 - effectivePenalty) : null;
  const rawCritical = findings.some((finding) => finding.base_severity === "critical" && finding.raw_score_impact > 0);
  const effectiveCritical = findings.some(
    (finding) => finding.effective_severity === "critical" && !finding.suppressed && finding.score_impact > 0
  );
  if (rawCritical && rawScore !== null) rawScore = Math.min(rawScore, 50);
  if (effectiveCritical && effectiveScore !== null) effectiveScore = Math.min(effectiveScore, 50);
