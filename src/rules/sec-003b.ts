...
export const sec003b: RuleModule = {
  id: "MCP-SEC-003b",
  ...
  check(context) {
    ...
    if (/health|metrics|docs/i.test(content) && !/tool|mcp|messages|sse|rpc/i.test(content)) return [];
    if (
      /app\.use\s*\(\s*["'`]\/?(?:mcp|messages?|sse|rpc|tools?)[^"'`]*["'`]\s*,\s*(?:requireAuth|authMiddleware|verifyToken|verifyJwt|protect|guard)/i.test(
        content
      ) ||
      /(?:app|router|fastify)\.(?:post|get|all|route|use)\s*\([^)]*["'`]\/?(?:mcp|messages?|sse|rpc|tools?)[^"'`]*["'`][^)]*,\s*(?:requireAuth|authMiddleware|verifyToken|verifyJwt|protect|guard)/i.test(
        content
      )
    ) {
      return [];
    ...
    if (!handlerLocal && hasAuthLikeGuard(content)) return [];
