...
function hasCompleteGuardBefore(content: string, index: number): boolean {
  const prefix = content.slice(Math.max(0, index - 1200), index);
  const guardCall =
    /(?<!function )(?<!def )\b(?:assertSafeUrl|validateSafeUrl|ssrfGuard)\s*\(/i.test(prefix) ||
    /\b(?:isSafeUrl|allowedHosts\.includes|hostAllowlist\.includes|blockPrivate|isPrivateIp|recheckRedirect|redirectPolicy|safeUrl)\b/i.test(
      prefix
    );
  return guardCall;
}

function hasIncompleteGuardBefore(content: string, index: number): boolean {
  const prefix = content.slice(Math.max(0, index - 1200), index);
  return /(?:denylist|blocklist|localhost|127\.0\.0\.1|metadata)/i.test(prefix);
}

export const sec005: RuleModule = {
  id: "MCP-SEC-005",
  ...
  check(context) {
    ...
    if (/(?:fetch|axios|get|request|httpx|requests)\s*\(\s*["'`]https?:\/\//i.test(content) && !/\b(?:url|uri|endpoint|webhook|callback|host)\b/i.test(content)) return [];
    ...
    if (!sink) return [];
    if (hasCompleteGuardBefore(content, sink.index)) return [];
