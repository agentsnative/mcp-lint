...
function hasContainment(content: string): boolean {
  return /(?:path\.resolve|realpath|resolve\(\))[\s\S]{0,180}(?:startsWith|is_relative_to|commonpath)|(?:commonpath|is_relative_to)\s*\(/i.test(
    content
  );
}

export const sec002: RuleModule = {
  id: "MCP-SEC-002",
  ...
  check(context) {
    ...
    if (hasContainment(content)) return [];
