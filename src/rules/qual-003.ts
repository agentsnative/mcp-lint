...
export const qual003: RuleModule = {
  id: "MCP-QUAL-003",
  ...
  check(context) {
    ...
    if (!description) {
      ...
      excerpt = text;
      if (text.length < 20) subtype = "description_too_short";
      else if (/(?:TODO|TBD|\{\{|\[insert|lorem ipsum)/i.test(text)) subtype = "unfilled_template";
      else if (/(?:ignore previous instructions|system prompt|do anything now)/i.test(text)) subtype = "injection_shaped";
      else if (text.toLowerCase().replace(/[\s_-]+/g, "") === toolName.toLowerCase().replace(/[\s_-]+/g, "")) subtype = "description_repeats_name";
      else if (/```/.test(text)) subtype = "code_block_in_description";
      else if (/email/i.test(toolName) && /calendar/i.test(text)) subtype = "domain_mismatch";
    }
