...
const runtimeIgnores = [
    ...
    {
        reason: "build_config",
        test: (relativePath) => /(?:^|\/)[^/]*\.config\.[cm]?[jt]sx?$/i.test(relativePath) ||
            /(?:^|\/)(?:rollup|vite|webpack|esbuild|tsup|jest|vitest|babel|eslint|prettier|tailwind|postcss)\.config\.[cm]?[jt]sx?$/i.test(relativePath) ||
...
function detectedLanguageFor(filePath) {
    if (languageFor(filePath))
        return languageFor(filePath);
    ...
    if (/\.java$/i.test(filePath))
        return "java";
    if (/\.(?:cs|fs|vb)$/i.test(filePath))
        return "csharp";
    if (/\.rb$/i.test(filePath))
        return "ruby";
    if (/\.php$/i.test(filePath))
        return "php";
    if (/\.kt$/i.test(filePath))
        return "kotlin";
    if (/\.swift$/i.test(filePath))
        return "swift";
    if (/\.(?:c|cc|cpp|cxx|h|hpp)$/i.test(filePath))
        return "cpp";
    return undefined;
}
function runtimeIgnoreReason(relativePath) {
...
function walk(root, current = root) {
    ...
    for (const entry of entries) {
        if (entry.isDirectory()) {
            if (hardIgnoredDirs.has(entry.name))
                continue;
...
function loadSourceFiles(root, config) {
    ...
    const detectedUnsupported = detectedLanguages.some((language) => !supportedLanguages.includes(language));
...
function applyInlineSuppressions(findings, files) {
...
export function scanPath(targetPath, options = {}) {
    ...
    const rawFindings = coverage.status === "evaluated" ? sourceFiles.flatMap((file) => rules.flatMap((rule) => rule.check({ root, file }))) : [];
    const suppressed = applyInlineSuppressions(rawFindings, sourceFiles);
    const findings = applyPresentation(suppressed, config).sort((a, b) => `${a.location.path}:${a.location.line}:${a.rule_id}:${a.subtype ?? ""}`.localeCompare(`${b.location.path}:${b.location.line}:${b.rule_id}:${b.subtype ?? ""}`));
