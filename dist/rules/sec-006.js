...
const concreteCorsAllowlistPattern = /(?:cors|CORSMiddleware)\s*\([\s\S]{0,240}(?:origin|allow_origins)\s*[:=]\s*\[\s*["'`](?!\*)[^"'`]+["'`]/i;
const namedOriginMiddlewarePattern = /(?:app|router|fastify)\.use\s*\(\s*(?:[A-Za-z_$][\w$]*(?:Origin|Cors|Csrf|Rebinding)[\w$]*|(?:validate|require|enforce|check)Origin[\w$]*)\s*\)/i;
const routeLevelOriginMiddlewarePattern = /,\s*(?:[A-Za-z_$][\w$]*(?:Origin|Cors|Csrf|Rebinding)[\w$]*|(?:validate|require|enforce|check)Origin[\w$]*)\s*(?:,|\))/i;
const explicitOriginGatePattern = /(?:app|router|fastify)\.use\s*\([\s\S]{0,700}(?:(?:allowedOrigins|trustedOrigins|allowlist|localhost|127\.0\.0\.1|csrf|rebinding)[\s\S]{0,220}(?:headers?\.origin|headers\s*\[\s*["'`]origin["'`]\s*\]|headers\.get\(["'`]origin["'`]\))|(?:headers?\.origin|headers\s*\[\s*["'`]origin["'`]\s*\]|headers\.get\(["'`]origin["'`]\))[\s\S]{0,220}(?:allowedOrigins|trustedOrigins|allowlist|localhost|127\.0\.0\.1|csrf|rebinding))[\s\S]{0,240}(?:status\s*\(\s*403|sendStatus\s*\(\s*403|HTTPException|raise|throw|return\s+res)/i;
function hasConcreteOriginGuardBeforeRoute(content, routeIndex, routeText) {
    const beforeRoute = content.slice(0, routeIndex);
    return (concreteCorsAllowlistPattern.test(beforeRoute) ||
...
function hasDynamicCorsBeforeRoute(content, routeIndex) {
    const beforeRoute = content.slice(0, routeIndex);
...
export const sec006 = {
    ...
    check(context) {
        ...
        if (!route)
            return [];
        if (hasConcreteOriginGuardBeforeRoute(content, route.index, route.match[0]))
            return [];
